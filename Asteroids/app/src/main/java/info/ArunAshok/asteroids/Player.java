...
public class Player extends View {
    ...
    ArrayList<Placement> jl = new ArrayList<Placement>(); // Array for space junk objects
    ArrayList<Shot> ml = new ArrayList<Shot>(); // Array for missile objects
    ArrayList<Shot> dm = new ArrayList<Shot>(); // Array for missile objects to be deleted
    ...
    int maxDecayRate = 8000, minDecayRate = 6000; // The random decay rate ranges for space junk
    ...
    int junkDecayRate = 7000; // Time for space junk to move Coordinates
    ...
    public void onDraw(Canvas c) {
        ...
        // Spaceship shadow, in space where there is no surface to cast a shadow on
        // but this is a game, and games are not always realistic so deal with it or
        // shut it off
        if (spaceshipShadow) {
            pt.set(cx + 3, cy + 4);
        ...
        if (SystemClock.uptimeMillis() - timePassed > junkDecayRate) {
            ...
            Placement j = jl.get(junkSelector);
            jl.remove(j); // Die
            jl.add(new Placement()); // Reborn!
            if (junkSelector >= jl.size() - 1) {
                junkSelector = 0;
            } else {
                junkSelector++;
            }
        ...
        // Creates new missile, sets the missile initial coordinates and adds to list of
        // missiles
        fireInTheHole();
        ...
        for (Shot m : ml) {
            ...
            for (Placement j : jl) {
                // Checks if the coordinates of center of the missile are within the area of the
                // space junk
            ...
            if (dualFire && (dr == Direction.NORTH | dr == Direction.SOUTH)) {
                c.drawCircle(m.mx + r, m.my, m.mr, paint);
            }
            if (dualFire && (dr == Direction.EAST | dr == Direction.WEST)) {
                c.drawCircle(m.mx, m.my + r, m.mr, paint);
            }

            // Call the missile's instance of lightSpeed()
            // This function moves the missile in the direction it was facing until it hits
            // a wall
            m.lightSpeed();
    ...
    // Draws path of an equilateral triangle(Spaceship) at provided point to size of
    // the provided width
