...
import info.ArunAshok.asteroids.R;

public class Asteroids extends AppCompatActivity implements View.OnTouchListener {
    private boolean misPlaying = true;
    private Music mServ = new Music();
    ...
    // Updates the canvas, checks for updated score and checks for playing the
    // explosion sound
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            switch (btn) {
                                case 1:
                                    Player.cx += Player.vl;
                                    Player.dr = Player.Direction.EAST;
                                    break;
                                case 2:
                                    Player.cx -= Player.vl;
                                    Player.dr = Player.Direction.WEST;
                                    break;
                                case 3:
                                    Player.cy -= Player.vl;
                                    Player.dr = Player.Direction.NORTH;
                                    break;
                                case 4:
                                    Player.cy += Player.vl;
                                    Player.dr = Player.Direction.SOUTH;
                                    break;
                            }
                        }
                    });
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            switch (v.getId()) {
                ...
                case R.id.timer:
                    Player.score = 0;
                    updateScore();
                    Log.d("Action", "Timer button");
                    new CountDownTimer(60000, 1000) {

                        TextView tv1 = (TextView) findViewById(R.id.timer);

                        public void onTick(long millisUntilFinished) {
                            tv1.setText("Timer: " + millisUntilFinished / 1000);
                        }

                        public void onFinish() {
                            tv1.setText("Final: " + Player.score);
                        }
                    }.start();
